...
export default async function getCountryDetail(code) {
  ...
  if (countryDetail.currencies.length > 0) {
    countryDetail.currencies = countryDetail.currencies.map(currency => currency.name);
  }
  if (countryDetail.languages.length > 0) {
    countryDetail.languages = countryDetail.languages.map(language => language.name);
  }
